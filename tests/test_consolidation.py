...
需要环境变量 XACODE_TEST_API_KEY、XACODE_TEST_BASE_URL、XACODE_TEST_MODEL。
...
def test_lock_first_acquire():
    from xacode.memory.consolidation import _read_last_consolidated_at, _try_acquire_lock
...
def test_lock_blocks_when_held():
    from xacode.memory.consolidation import _try_acquire_lock
...
def test_lock_rollback_deletes_on_zero():
    from xacode.memory.consolidation import _try_acquire_lock, _rollback_lock
...
def test_prompt_contains_all_phases():
    from xacode.memory.consolidation import _build_consolidation_prompt
...
def test_do_consolidation_builds_subagent(monkeypatch):
    ...
    from xacode.memory.consolidation import MemoryConsolidator
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".xacode", "memory")
...
def test_do_consolidation_subagent_inherits_rules(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".xacode", "memory")
        setup_test_memories(mem_dir)
        rules_file = Path(work_dir) / ".xacode" / "permissions.yaml"
...
def test_do_consolidation_subagent_opens_user_memory_dir(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        ...
        fake_home = Path(os.path.abspath(os.sep)) / "fake-user-home-for-xacode-test"
        consolidator._user_mem_dir = str(fake_home / ".xacode" / "memory")
        ...
        user_mem_file = fake_home / ".xacode" / "memory" / "MEMORY.md"
...
@pytest.mark.skipif(
    not os.environ.get("XACODE_TEST_API_KEY"),
    reason="XACODE_TEST_API_KEY not set"
)
@pytest.mark.timeout(120)
def test_e2e_consolidation_merges_duplicates():
    api_key = os.environ["XACODE_TEST_API_KEY"]
    base_url = os.environ.get("XACODE_TEST_BASE_URL", "https://api.minimaxi.com/v1")
    model = os.environ.get("XACODE_TEST_MODEL", "MiniMax-M3")
...
async def _run_consolidation(work_dir, api_key, base_url, model, mem_dir):
    ...
    from xacode.permissions.checker import PermissionChecker
    from xacode.tools import ToolRegistry
    from xacode.tools.bash import Bash
    from xacode.tools.edit_file import EditFile
    from xacode.tools.glob import Glob
    from xacode.tools.grep import Grep
    from xacode.tools.read_file import ReadFile
    from xacode.tools.write_file import WriteFile

    from xacode.config import ProviderConfig
    from xacode.client import OpenAICompatClient
    ...
    from xacode.permissions.rules import RuleEngine
    from xacode.permissions.dangerous import DangerousCommandDetector
    from xacode.permissions.checker import PermissionMode
