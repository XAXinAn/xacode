"""记忆子系统测试：指令 include 展开、会话记录读写与记忆管理。"""
...
from xacode.conversation import (
    ConversationManager,
    Message,
    ToolResultBlock,
    ToolUseBlock,
...
from xacode.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
from xacode.memory.session import (
    ResumeResult,
    Session,
    SessionManager,
    SessionMeta,
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        xacode_md = tmp_path / "XACODE.md"
        xacode_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """同一目录下 XACODE.md 在前，.xacode/XACODE.md 在后（优先级更高）。"""
        root_md = tmp_path / "XACODE.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".xacode"
        dotdir.mkdir()
        dot_md = dotdir / "XACODE.md"
    ...
    def test_dotdir_walks_up(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """.xacode/XACODE.md 参与逐级遍历，深层目录的排在后面。"""
        subprocess.run(["git", "init"], cwd=tmp_path, capture_output=True)
        sub = tmp_path / "pkg" / "deep"
        sub.mkdir(parents=True)
        (tmp_path / ".xacode").mkdir(exist_ok=True)
        (tmp_path / ".xacode" / "XACODE.md").write_text("dotdir root", encoding="utf-8")
        (sub / ".xacode").mkdir()
        (sub / ".xacode" / "XACODE.md").write_text("dotdir leaf", encoding="utf-8")
...
class TestSession:
    def test_append_writes_jsonl_and_updates_meta(self, tmp_path: Path) -> None:
        sessions_dir = tmp_path / ".xacode" / "sessions"
...
class TestMemoryManager:
    ...
    def test_load_includes_memory_index(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 创建项目级记忆目录和文件
        project_mem_dir = tmp_path / "project" / ".xacode" / "memory"
    ...
    def test_load_all(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 用户级记忆
        user_mem_dir = fake_home / ".xacode" / "memory"
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "xacodeMd" in conv.history[1].content
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "xacodeMd" in conv.history[0].content
    ...
    def test_inject_skills_only(self) -> None:
        """项目可能没写 XACODE.md 也没有记忆，只有 Skill 时同样要注入。"""
        conv = ConversationManager()
...
class TestMemoryExtraction:
    def test_memory_types_aligned_with_go(self, tmp_path: Path) -> None:
        """验证四种记忆类型枚举。"""
        from xacode.memory.auto_memory import VALID_TYPES, _USER_LEVEL_TYPES, _PROJECT_LEVEL_TYPES
