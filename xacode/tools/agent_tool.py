"""Agent 工具：派生子 agent（前台或后台）并回收其结果。"""
...
if TYPE_CHECKING:
    ...
    from xacode.client import LLMClient
...
class AgentTool(Tool):
    ...
    def _inherited_rule_engine(self) -> Any:
        ...
        父级没有权限检查器时退化为空规则集。
        """
        from xacode.permissions import RuleEngine
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        from xacode.agents.fork import ForkError, build_forked_messages
        from xacode.agents.parser import AgentDef
        from xacode.agents.tool_filter import clone_registry_for_fork, resolve_agent_tools
        from xacode.agent import Agent as AgentClass
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        from xacode.agents.fork import ForkError, build_forked_messages
        from xacode.agents.parser import AgentDef
        from xacode.agents.tool_filter import build_teammate_tools
        from xacode.agent import Agent as AgentClass
        from xacode.conversation import ConversationManager
        from xacode.permissions import (
            DangerousCommandDetector,
        ...
        from xacode.teams.models import BackendType, TeammateInfo
    ...
    def _spawn_pane_teammate(
        ...
        from xacode.teams.spawn import build_teammate_cli
        ...
        if mailbox is not None and p.prompt:
            from xacode.teams.mailbox import create_message
            from xacode.teams.spawn_inprocess import LEAD_NAME
            mailbox.write(
                teammate_name,
                create_message(
        ...
        # 构造把本 xacode 拉起为队友 worker 模式的命令，cd 到该队友的 worktree
        cli_command = build_teammate_cli(p.team_name, teammate_name, wt.path)

        try:
            if backend == BackendType.TMUX:
                from xacode.teams.spawn_tmux import spawn_tmux_teammate
                ...
                from xacode.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        ...
        from xacode.worktree.integration import (
    ...
    def _create_client_for_model(self, model_alias: str) -> LLMClient | None:
        ...
        from xacode.client import create_client
