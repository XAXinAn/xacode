"""命令行入口：装配配置、Hooks 与权限模式，并分发到 TUI、-p 非交互、remote 与 teammate 四种运行方式。"""
...
from xacode.config import ConfigError, load_config
from xacode.hooks import HookConfigError, HookEngine, load_hooks
from xacode.permissions import PermissionMode


def main() -> None:
    # 队友 worker 模式：由 tmux/iTerm2 窗格用 `-m xacode --teammate ...` 拉起。
    ...
    # 先确保 .xacode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".xacode").mkdir(parents=True, exist_ok=True)
    # 追加写：排查异常退出要看的往往是上一次运行的日志，覆盖会把现场冲掉
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".xacode/debug.log",
        filemode="a",
    )
    crashlog.install()

    parser = argparse.ArgumentParser(prog="xacode", description="XaCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from xacode.remote import RemoteServer
    ...
    from xacode.app import XaCodeApp
    from xacode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from xacode.client import create_client, resolve_context_window
    from xacode.conversation import ConversationManager
    from xacode.memory.instructions import load_instructions
    ...
    from xacode.tools import create_default_registry
    ...
    if config.mcp_servers:
        from xacode.mcp import MCPManager
...
async def _build_teammate_registry(
    ...
    from xacode.tools.send_message import SendMessageTool
    from xacode.tools.synthetic_output import SyntheticOutputTool
    from xacode.tools.task_create import TaskCreateTool
    from xacode.tools.task_get import TaskGetTool
    from xacode.tools.task_list import TaskListTool
    from xacode.tools.task_update import TaskUpdateTool
    from xacode.worktree import WorktreeManager
...
async def _run_teammate(team_name: str, agent_name: str) -> None:
    ...
    """
    from xacode.agent import Agent
    from xacode.client import create_client, resolve_context_window
    from xacode.memory.instructions import load_instructions
    from xacode.permissions import (
        DangerousCommandDetector,
    ...
    from xacode.teams.spawn_inprocess import LEAD_NAME, spawn_inprocess_teammate
    ...
    checker = PermissionChecker(
        ...
        sandbox=PathSandbox(work_dir),
        rule_engine=RuleEngine(
            user_rules_path=Path.home() / ".xacode" / "permissions.yaml",
            project_rules_path=Path(work_dir) / ".xacode" / "permissions.yaml",
            local_rules_path=Path(work_dir) / ".xacode" / "permissions.local.yaml",
        ),
