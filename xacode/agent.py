"""Agent 主循环：消费 LLM 的流式输出、并发执行工具调用，并串起权限审批与 Hook 引擎。"""
...
from xacode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from xacode.conversation_pairing import REJECTED_TOOL_RESULT
from xacode.conversation import ThinkingBlock as ConvThinkingBlock
from xacode.memory.auto_memory import MemoryManager
from xacode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from xacode.hooks import HookContext, HookEngine, ToolRejectedError
from xacode.hooks.engine import HookNotification
from xacode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from xacode.tools import ToolRegistry
from xacode.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _announce_deferred_tools(self, conversation: ConversationManager) -> None:
        ...
        from xacode.mcp.loading_strategy import McpLoadingMode
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".xacode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".xacode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from xacode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(
        ...
        from xacode.context.manager import (
