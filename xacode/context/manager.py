"""上下文管理：工具结果落盘与预算裁剪、compact 阈值与摘要生成、恢复附件构建。"""
...
def spill_dir(work_dir: str, session_id: str = "") -> Path:
    """溢写目录：按会话隔离在 .xacode/sessions/<会话id>/tool-results 下，
    会话 id 为空（一次性调用、测试）时落到 default。"""
    sid = session_id or "default"
    return Path(work_dir) / ".xacode" / "sessions" / sid / "tool-results"
...
async def _call_summary_with_cache_sharing(
    client: Any,
    ...
    消息前缀和主对话上一次 API 调用一致，能命中 Prompt Cache
    （Anthropic 90% 折扣、OpenAI 50% 折扣、DeepSeek ~90% 折扣）。
    PTL 时返回 None，由调用方降级到文本序列化 + 截断重试路径。
    """
    from xacode.tools.base import TextDelta
    ...
    try:
        collected_text = ""
        async for event in client.stream(summary_conv, system="", tools=tool_schemas):
...
async def _call_summary_with_ptl_retry(
    client: Any,
    prefix: list[Message],
    tool_schemas: list[Mapping[str, Any]] | None,
    max_retries: int = 3,
    ...
    把前缀消息序列化为纯文本，和摘要指令拼成一条 user message 发给 LLM。
    PTL 时丢弃最旧的 API-round 分组再重试，最多 max_retries 次。
    """
    from xacode.tools.base import TextDelta
...
async def auto_compact(
    ...
    try:
        summary = await _call_summary_with_cache_sharing(
            client, effective_history, tool_schemas
        )
    except Exception as e:
    ...
    if summary is None:
        # PTL 降级：文本序列化 + 截断重试
        try:
            summary = await _call_summary_with_ptl_retry(
                client, to_summarize, tool_schemas
            )
        except Exception as e:
            if breaker is not None:
                breaker.record_failure()
