"""配置加载：合并全局与项目级 config.yaml，产出 provider、MCP、worktree、sandbox 等配置。"""
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".xacode" / "config.yaml",
        cwd / ".xacode" / "config.yaml",
        cwd / ".xacode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .xacode/config.yaml "
            "in project or ~/.xacode/config.yaml"
        )
    return merged
