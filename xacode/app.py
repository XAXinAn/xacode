"""Textual TUI 主应用：装配消息区、输入框、工具调用块，并处理会话级交互。"""
...
from xacode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from xacode import crashlog
from xacode.commands.completion import CompletionPopup
from xacode.commands.handlers import register_all_commands
from xacode.config import MCPServerConfig, ProviderConfig
from xacode.hooks import HookContext, HookEngine, load_hooks
from xacode.conversation import ConversationManager, Message
from xacode.mcp import ConnectResult, MCPManager
from xacode.mcp.tool_wrapper import mcp_tool_name_prefix
from xacode.memory import (
    MemoryManager,
    Session,
    SessionManager,
    RecallResult,
...
from xacode.permissions import (
...
from xacode.agents.task_manager import TaskManager
from xacode.agents.trace import TraceManager
from xacode.agents.notification import inject_task_notifications
from xacode.commands.handlers.tasks import create_tasks_command
from xacode.skills.executor import SkillExecutor
from xacode.skills.loader import SkillLoader
from xacode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from xacode.tools import ToolRegistry, create_default_registry
from xacode.tools.agent_tool import AgentTool
from xacode.tools.ask_user import AskUserEvent, AskUserTool
from xacode.tools.impl.tool_search import ToolSearchTool
from xacode.tools.mcp_call import McpCallTool
from xacode.tools.install_skill import InstallSkillTool
from xacode.tools.load_skill import LoadSkill
from xacode.worktree.cleanup import start_stale_cleanup_task
from xacode.worktree.manager import WorktreeManager
from xacode.commands.handlers.worktree import create_worktree_command
from xacode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".xacode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".xacode" / "history"
...
class XaCodeApp(App):
    ...
    def __init__(
        ...
        from xacode.config import SandboxAppConfig
    ...
    # 启动横幅左侧的 ASCII 猫。每行都会按最大宽度补齐后再拼右侧文字，
    # 所以这里不必手工对齐空格。
    _BANNER_ART = (
        r"  __    __",
        r"o-''))_____\\",
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        art = XaCodeApp._BANNER_ART
        width = max(len(line) for line in art) + 2
        # 前三行右侧依次是版本、模型、工作目录；第四行只放猫。
        captions = ("XaCode v0.1.0", model, work_dir)
        text = RichText()
        for index, line in enumerate(art):
            text.append(line.ljust(width), style="bold color(99)")
            caption = captions[index] if index < len(captions) else ""
            if caption:
                text.append(caption, style="color(242)")
            if index < len(art) - 1:
                text.append("\n")
        return text
    ...
    def on_mount(self) -> None:
        self.register_theme(_XACODE_THEME)
        self.theme = "xacode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".xacode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".xacode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".xacode" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from xacode.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.xacode/config.yaml",
                        f"{work_dir}/.xacode/permissions.local.yaml",
                    ],
        ...
        from xacode.filehistory import FileHistory
        ...
        from xacode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from xacode.config import WorktreeConfig
        ...
        from xacode.tools.enter_worktree import EnterWorktreeTool
        from xacode.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from xacode.teams.manager import TeamManager
        from xacode.tools.team_create import TeamCreateTool
        from xacode.tools.team_delete import TeamDeleteTool
        ...
        from xacode.commands.handlers.trace import create_trace_command
        ...
        from xacode.tools.send_message import SendMessageTool
        from xacode.tools.synthetic_output import SyntheticOutputTool
        from xacode.tools.task_stop import TaskStopTool
        ...
        if self._enable_coordinator_mode:
            from xacode.agents.tool_filter import apply_coordinator_filter
    ...
    async def _prefetch_relevant_memories(self, query: str) -> RecallResult:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from xacode.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from xacode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from xacode.plan_dialog import InlinePlanWidget, PlanChoice
        from xacode.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from xacode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from xacode.permission_dialog import InlinePermissionWidget
    ...
    async def _init_mcp(self) -> None:
        ...
        if self._selected_provider is not None:
            from xacode.mcp.loading_strategy import decide_and_apply
