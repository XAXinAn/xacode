"""项目指令加载：按全局、项目目录链、本地覆盖的顺序读取 XACODE.md / AGENTS.md 并展开 include。"""
...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.xacode/XACODE.md, ~/.xacode/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 XACODE.md、AGENTS.md
       和 .xacode/XACODE.md
    3. workDir/XACODE.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".xacode" / "XACODE.md")
    _add(home / ".xacode" / "AGENTS.md")

    # 2. 项目目录链
    for d in _project_instruction_dirs(root):
        _add(d / "XACODE.md")
        _add(d / "AGENTS.md")
        # .xacode/ 下的同名文件：想让指令进 .gitignore 的项目放这里
        _add(d / ".xacode" / "XACODE.md")

    # 3. 本地覆盖
    _add(root / "XACODE.local.md")
