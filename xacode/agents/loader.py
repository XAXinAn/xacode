"""Agent 定义加载：从项目 .xacode/agents 与 ~/.xacode/agents 扫描并解析 agent 定义。"""
...
PROJECT_AGENTS_DIR = ".xacode/agents"
USER_AGENTS_DIR = "~/.xacode/agents"
...
class AgentLoader:
    ...
    def _load_builtins(self) -> list[AgentDef]:
        results: list[AgentDef] = []
        try:
            builtins_pkg = importlib.resources.files("xacode.agents.builtins")
        ...
        for item in builtins_pkg.iterdir():
            ...
            try:
                ...
                from xacode.agents.parser import parse_frontmatter, _validate_agent_meta
