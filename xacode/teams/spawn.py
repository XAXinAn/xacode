"""teammate 启动命令构造：生成拉起独立进程所需的命令行。"""
...
def build_teammate_cli(team_name: str, member_name: str, workdir: str = "") -> str:
    ...
    用 sys.executable + `-m xacode` 复用当前
    Python 解释器和包，产出形如：

        cd <workdir> && <python> -m xacode --teammate --team-name <t> --agent-name <n>
    ...
    return (
        ...
        f"{shell_quote(python)} -m xacode --teammate "
