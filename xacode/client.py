"""LLM 客户端：Anthropic / OpenAI / OpenAI 兼容三种协议的流式调用、重试与 prompt cache 标记。"""
...
from xacode.mcp.loading_strategy import NATIVE_TOOL_SEARCH_BETA
from xacode.conversation_pairing import ensure_tool_pairing
from xacode.serialization import (
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .xacode/config.yaml or via ANTHROPIC_API_KEY env var."
            )
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .xacode/config.yaml or via OPENAI_API_KEY env var."
            )
