"""worktree 管理：创建 / 查询 / 删除 worktree 并维护其 git 状态。"""
...
from xacode.worktree.changes import (
    CleanupResult,
    Changes,
    count_worktree_changes,
    has_worktree_changes,
)
from xacode.worktree.models import Worktree, WorktreeSession
from xacode.worktree.session import load_worktree_session, save_worktree_session
from xacode.worktree.setup import perform_post_creation_setup
from xacode.worktree.slug import flatten_slug, validate_slug
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".xacode" / "worktrees"
        )
        self._xacode_dir = Path(repo_root) / ".xacode"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._xacode_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._xacode_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._xacode_dir)
        if session is None:
            return None
