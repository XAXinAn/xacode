"""worktree 会话持久化：把当前 worktree 会话写入 .xacode。"""
...
def _session_path(xacode_dir: Path) -> Path:
    return xacode_dir / SESSION_FILENAME
...
def load_worktree_session(xacode_dir: Path) -> WorktreeSession | None:
    path = _session_path(xacode_dir)
    if not path.exists():
        return None
    try:
