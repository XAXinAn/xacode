"""技能安装：从远端仓库拉取技能目录并写入 .xacode/skills。"""
...
async def _list_contents(
    ...
    headers = {
        ...
        "User-Agent": "xacode-install-skill",
    }
...
async def _fetch_blob(
    ...
    headers = {"User-Agent": "xacode-install-skill"}
...
def user_skills_root() -> Path:
    """返回 ~/.xacode/skills，不存在则自动创建。"""
    root = Path.home() / ".xacode" / "skills"
